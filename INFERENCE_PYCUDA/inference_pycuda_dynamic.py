...
def save_images2(images, predIMG, fname="test_engine.png", dir_path_id="./img_out_id/", dir_path_img="./img_out_img/", engine_img=False):
    ...
    #img_input =  img_input.transpose(1, 2, 0)
    img_input = np.squeeze(images, axis = 0).numpy()
...
def allocate_buffers(engine, batch_size):
    ...
    for binding in engine:
        ...
        dim2 = engine.get_binding_index(binding)
        print("dims",dims)
...
if __name__ == "__main__":
    ...
    total_time = 0; i = 0; s = 0; total_end_to_end = 0
    ...
    #valloader_gt = data.DataLoader(loader, batch_size=batch_size, num_workers=1)
    ...
    #context.set_binding_shape(batch_size, (3, 608, 608))
    ...
    if inference_with_CITY:
        for i, (images, labels, fname) in enumerate(valloader):
            ...
            ### --> when images less than batch_size USE.
            ...
            pred_id_val = trt_outputs[0].reshape(batch_size, proc_size[0], proc_size[1])
            
            if save_img:
                for b in range(batch_size):
                    save_images2(images[b], pred_id_val[b], fname=fname[b], dir_path_id="../pred/", dir_path_img="../pred/", engine_img=True)  
            
            elapsed_time_post = time.perf_counter() - start_time 

            total_time += elapsed_time
            total_end_to_end += (elapsed_time + elapsed_time_post)

            print("Time (iter {0:5d})--> Inference time:{1:3.5f}({2:3.5f}fps), Postprocess time:{3:3.5f}({4:3.5f}fps)".format(
                    i + 1, elapsed_time, batch_size/elapsed_time, elapsed_time_post, batch_size/elapsed_time_post
                )
            )
            i = i + 1
        ...
        print("i",i)
        print("Total Frame Rate inference time= {0:3.2f}({1:2.5f}fps)".format(total_time, i*batch_size/total_time))
        print("Total Frame Rate End-to-End(inference+post)time= {0:3.2f}({1:3.2f}fps)".format(total_end_to_end, i*batch_size/total_end_to_end))
        ...
        batch_img = np.concatenate((img1d,img1d,img1d), axis=0)

        np.copyto(inputs[0].host, batch_img)

        trt_outputs = do_inference(context, bindings=bindings, inputs=inputs, outputs=outputs, stream=stream)

        pred_id_val = trt_outputs[0] = trt_outputs[0].reshape(3, proc_size[0], proc_size[1])

        if save_img:
            filename = os.path.splitext(os.path.basename(single_test_img_path))[0]
            save_images2(img, pred_id_val[0], fname=filename, dir_path_id="../pred/", dir_path_img="../pred/", engine_img=True)  